...
// maxFileArgBytes is the maximum total byte-length of file-path arguments per
// tool invocation. Kept well below Linux ARG_MAX (~2 MB) to leave headroom for
// the binary path, flags, and environment variables.
const maxFileArgBytes = 200 * 1024

// splitBatches partitions files into slices whose total path length stays
// within maxFileArgBytes. A single-element result means no splitting was needed.
...
//
// When the total size of file-path arguments would exceed maxFileArgBytes, the
// file list is split into batches and the tool is invoked once per batch.
// Findings from all batches are merged. Tools that do not accept an explicit
// file list (NoBatch: true) are always invoked once.
...
func invokeBatch(ctx context.Context, def ToolDef, binary, workspace string, fix bool, files []string, log io.Writer) ([]Finding, string) {
	...
	if len(findings) == 0 && exitCode != 0 {
		// Non-zero exit with no parseable findings means the tool itself failed
		// (e.g. missing input file, bad config). Surface stderr for diagnosis.
